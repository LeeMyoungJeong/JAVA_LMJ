...
import Day15.Board_DAO.BoardDAO;
import Day15.Board_DTO.Board;

public class Main {
	static Board board;	
	static List<Board> boardList; // 동적 자료구조를 이제 다룰 수 있음.(컬렉션)
	static Scanner sc = new Scanner(System.in);
	static BoardDAO dao = new BoardDAO();
	
	public static void menu() { //메뉴판을 출력해주는 메소드
	...
	public static void main(String[] args) {
		...
		do {
			...
			default:
						System.out.println("(0~5)사이의 숫자를 입력해라!");
						break;
			}
		} while (true);
	}
	...
	private static void menuWrite() {
		...
		board = new Board(title, content, writer); // --> 3종류의 인자를 받아서 Board객체를 형성함.
		dao.insert(board); // ---> 얘가 데이터 삽입 요청 메소드임
	...
	private static void menuUpdate() { // 게시글 수정 메소드
	 ...
	 * - 그리고 게시글 목록 전체 출력 메소드(printAll 메소드)를 호출한다.
	 */
